...
namespace MarsIceEngine.Entity
{
    public class Camera
    {
        ...
        public Matrix TranslationMatrix
        {
            get
            {
                var translation = Matrix.CreateTranslation(-(int) Position.X, -(int) Position.Y, 0);
        ...
        // if shouldClampToMap is true the camera will try not to pan outside of the
        // bounds of the map.
        public void MoveCamera(Vector2 cameraMovement, bool shouldClampToMap = false)
        {
        ...
        {
            Position = CenteredPosition(position, true);
        }

        private Vector2 CenteredPosition(Position position, bool clampToMap = false)
        {
            var cameraPosition = new Vector2((float)position.X * Constants.WorldCellWidth,
                (float)position.Y * Constants.WorldCellHeight);

            var cameraCenteredOnTilePosition =
               new Vector2(cameraPosition.X + Constants.WorldCellWidth / 2,
                   cameraPosition.Y + Constants.WorldCellHeight / 2);
        ...
        {
            var cameraMax = new Vector2(Constants.WorldWidth * Constants.WorldCellWidth -
                (ViewportWidth / Zoom / 2),
                Constants.WorldHeight * Constants.WorldCellHeight -
        ...
        public void Update(InputHelper inputHelper)
        {
            ...
            //    cameraMovement.Y = 1;
            //}
            if (inputHelper.WasActionHappened(Act.ZoomIn))
            {
                AdjustZoom(0.25f);
            }
            else if (inputHelper.WasActionHappened(Act.ZoomOut))
            {
                AdjustZoom(-0.25f);
            }
